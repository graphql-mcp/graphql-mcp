 ...
 * Java equivalent of ToolPublisher + PolicyEngine in .NET.
 */
public class GraphQLToMCPToolMapper {
    ...
    private String transformName(CanonicalOperation op) {
        String base = switch (config.namingPolicy()) {
            case VERB_NOUN -> op.operationType() == OperationType.QUERY
    ...
    private MCPToolDescriptor toDescriptor(CanonicalOperation op) {
        ...
        String description = prefix + (op.description() != null ? op.description()
                : "GraphQL " + op.operationType().name().toLowerCase() + ": " + op.graphQLFieldName());
