...
/**
 * Introspects a GraphQL schema and extracts canonical operations. This is the Java equivalent of
 * SchemaCanonicalizer in .NET.
 */
public class GraphQLSchemaIntrospector {
  ...
  private CanonicalOperation mapField(GraphQLFieldDefinition field, OperationType type) {
    List<CanonicalArgument> args = field.getArguments().stream().map(this::mapArgument).toList();

    return new CanonicalOperation(
        field.getName(), field.getDescription(), type, field.getName(), args);
  }
  ...
  public record CanonicalArgument(
      String name, String description, boolean required, Object defaultValue) {}
}
