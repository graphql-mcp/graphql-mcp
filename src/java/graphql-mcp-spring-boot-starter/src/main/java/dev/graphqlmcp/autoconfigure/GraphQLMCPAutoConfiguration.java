...
/**
 * Spring Boot auto-configuration for graphql-mcp.
 * Activates when Spring GraphQL is on the classpath and graphql.mcp.enabled=true.
 */
@Configuration
@ConditionalOnClass(GraphQLSchema.class)
@ConditionalOnProperty(prefix = "graphql.mcp", name = "enabled", havingValue = "true", matchIfMissing = true)
...
public class GraphQLMCPAutoConfiguration {
    ...
    public GraphQLToMCPToolMapper graphQLToMCPToolMapper(GraphQLMCPProperties properties) {
        var config = new GraphQLMCPConfig(
    ...
    public GraphQLMCPServer graphQLMCPServer(
            ...
            GraphQLSchemaIntrospector introspector,
